...
function comparelinks(expectedlinks) {
  if (CurrentLinks.length == expectedlinks.length) {
    ...
    try {
      // Check if CurrentLinks have all links of ExpectedLinks_DIEL
      ...
      allLinks.MissingLinks = CurrentLinks.filter(
        (link) => !expectedlinks.includes(link)
      )
      allLinks.NewLinks = expectedlinks.filter(
        (link) => !CurrentLinks.includes(link)
      )
...
describe('QATEST-97047 - should Compare URL in production and staging', () => {
  ...
  it(
    'should visits all links in DIEL countires and compare links with pre-defined list',
    { tags: ['@full-tests', '@row-tests'] },
    () => {
      comparelinks(allLinks.ExpectedLinks_DIEL)
    }
  )
})
