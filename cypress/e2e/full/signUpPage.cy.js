...
function checkSignUpPage() {
  cy.findByRole('heading', { name: 'Join over 2.5 million traders' }).should(
    'be.visible'
  )
  ...
  cy.findByRole('link', { name: 'Security and privacy policy' }).should(
    'be.visible'
  )
...
function verificationIsSent() {
  ...
  cy.findByRole('link', { name: "Didn't receive your email?" }).should(
    'be.visible'
  )
}
...
describe('QATEST-1613 - should validate the Sign Up page on desktop', () => {
  it(
    'should be able to navigate to Sign Up page for ROW',
    ...
    () => {
      ...
      it(
        'should be able to navigate to Sign Up page for EU',
