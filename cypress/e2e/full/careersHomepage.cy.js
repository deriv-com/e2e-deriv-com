...
function careersHomepage(size, region)
{
    ...
    if(size == 'desktop')
    {
        ...
        cy.findByRole('navigation').find('div').contains('Explore jobsExplore jobs').should('be.visible').findByRole('img').click()
        cy.findByRole('link', { name: 'Locations' }).click()
        cy.findByRole('navigation').find('div').contains('Explore jobsExplore jobs').should('be.visible').findByRole('img').click()
        cy.findByRole('link', { name: 'Home' }).click()
    }
    ...
    cy.findByRole('img', { name: 'Team discussing ideas', timeout: 30000 }).should('be.visible')

    cy.findByRole('heading', { name: "Join your team" }).should('be.visible')
    const imageNavigation = (imageName) => {
        cy.findByRole('img', { name: imageName }).should('exist').click().then(() => {
            cy.wait(2000)
            cy.go('back')
      })
    }
    ...
    cy.findByRole('heading', { name: 'Everything is built around our values' }).should('be.visible')
    ...
    cy.findByRole('img', { name: 'Group of people doing yoga' }).should('be.visible')
    cy.findByRole('img', { name: 'Water rafting team building activity' }).should('be.visible')
    cy.findByRole('heading', { name: "In the words of our employees" }).should('be.visible')
    ...
    cy.findByRole('heading', { name: 'Make an impact. Start your Deriv journey now.' }).should('be.visible')
