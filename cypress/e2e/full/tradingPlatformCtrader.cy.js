...
function cTrader_page() {
  ...
  cy.findByRole('img', {
    name: 'Deriv ctrader online trading platform',
  }).should('be.visible')
  cy.findByRole('heading', {
    name: 'What is Deriv cTrader',
    exact: true,
  }).should('be.visible')
  cy.findByRole('heading', {
    name: 'How to get started with a Deriv cTrader account',
  }).should('be.visible')

  const Verify = (buttonName, message) => {
    cy.findByRole('button', { name: buttonName }).click()
    cy.contains(
      `2. Go to the Trader’s hub and choose the ${message} option.`
    ).should('exist')
  }
  ...
  toggleAccordion(
    'What’s the minimum amount to open a position on Deriv cTrader?'
  )
...
describe('QATEST-23425 - should validate the cTrader page in desktop', () => {
  it(
    'should be able to navigate to cTrader page from home page and validate the page content and links in Desktop',
    { tags: ['@full-tests', '@row-tests'] },
    () => {
