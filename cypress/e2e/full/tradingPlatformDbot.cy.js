...
function Dbot_page() {
  ...
  cy.findByRole(
    'heading',
    { name: 'Build your strategy visually' },
    { timeout: 5000 }
  ).should('be.visible')
  cy.findByRole('img', { name: 'Build your bot using drag and drop' }).click()
  cy.findByRole(
    'heading',
    { name: 'Automate your trading ideas without writing code' },
    { timeout: 10000 }
  ).should('be.visible')

  for (let index = 0; index < 2; index++) {
    cy.findAllByText('Create free demo account', { timeout: 5000 })
      .eq(index)
      .click()
  ...
  for (let index = 0; index < 2; index++) {
    cy.findAllByRole('link', { name: 'Go to live demo' }, { timeout: 5000 })
      .eq(index)
...
describe('QATEST-1548 - should validate the Dbot page in responsive', () => {
  it(
    'should be able to navigate to Dbot page from home page and validate the page content and links in Mobile',
...
describe('QATEST-1541 - should validate the Dbot page in desktop', () => {
  it(
    'should be able to navigate to Dbot page from home page and validate the page content and links in Desktop',
    { tags: ['@full-tests', '@row-tests'] },
    () => {
