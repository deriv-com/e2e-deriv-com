...
const languageDetails = {
  ...
  101582: {
    ...
    redirectionVerify:
      'Schließen Sie sich über 2,5 Millionen Online-Händlern an',
    desktopVerify: 'DE',
  },
  101583: {
    urlCode: 'si',
    ...
    redirectionVerify:
      'මිලියන 2.5 කට අධික අන්තර්ජාල ගනුදෙනුකරුවන් සමඟ සම්බන්ධ වන්න',
    desktopVerify: 'SI',
  },
}
...
const redirectionRules = ['', '/', '?x=2', '/?x=2']
Object.entries(languageDetails).forEach((lang) => {
  describe(`QATEST-${lang[0]} - Language Selection is working:${lang[1].testLanguage}`, () => {
    ...
    screenSizes.forEach((screenSize) => {
      it(
        `it should verify language selection from UI on ${screenSize} size`,
        ...
        () => {
          ...
          if (screenSize == 'small') {
            ...
            cy.get('[data-cy="desktop-header"]')
              .contains('EN')
              .trigger('mouseover', { timeout: 1000 })
            cy.contains(lang[1].languageSelector).click()
            cy.location('href').should('contain', `/${lang[1].urlCode}`)
            cy.get('[data-cy="desktop-header"]')
              .find('.items-center')
              .should('contain.text', lang[1].desktopVerify)
          }
        }
      )
    })
