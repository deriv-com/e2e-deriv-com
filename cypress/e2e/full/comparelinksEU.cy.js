...
function comparelinks(expectedlinks) {
  if (CurrentLinks.length == expectedlinks.length) {
    ...
    try {
      // Check if CurrentLinks have all links of ExpectedLinks_EU
...
describe('QATEST-97047 - should Compare URL in production and staging', () => {
  ...
  it(
    'should visits all links in EU countires and compare links with pre-defined list',
    { tags: ['@full-tests', '@eu-tests'] },
    () => {
      comparelinks(allLinks.ExpectedLinks_EU)
    }
  )
})
