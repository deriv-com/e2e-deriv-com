...
function validate_derivxpage(region) {
  ...
  cy.findByRole('heading', { name: 'Why trade with Deriv X' }).should(
    'be.visible'
  )
  cy.findByRole('heading', { name: 'New and promising' }).should('be.visible')
  cy.findByRole('heading', {
    name: 'Multiple markets on a single platform',
  }).should('be.visible')
  cy.findByRole('heading', { name: '24/7 trading' }).should('be.visible')
  cy.findByRole('heading', { name: 'Bespoke trading experience' }).should(
    'be.visible'
  )
  ...
  cy.findByRole('img', { name: 'Know the margin impact on your CFD trading' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByRole('img', { name: 'Multiple charts to view your online trading' })
    .scrollIntoView()
    .should('be.visible')

  cy.findByRole('heading', {
    name: 'How to get started with a Deriv X account',
  })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText(
  ...
  cy.findByRole('img', { name: 'Deriv X demo account signup page' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText('2. Add a Deriv X demo account.').click()
  cy.findByRole('img', {
    name: 'Deriv X dashboard showing demo account comparison',
  })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText(
    '3. Start trading on the mobile app or through your web browser.'
  ).click()
  cy.findByRole('img', { name: 'Deriv X trading dashboard' })
    .scrollIntoView()
    .should('be.visible')
  ...
  cy.findByRole('img', { name: 'Sign in' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText('2. Add a Deriv real account.').click()
  cy.findByRole('img', { name: 'real account' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText('3. Add a Deriv X real account.').click()
  cy.findByRole('img', { name: 'Download the app' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText(
    '4. Start trading on the mobile app or through your web browser.'
  ).click()
  cy.findByRole('img', { name: 'Trading', exact: true })
    .scrollIntoView()
    .should('be.visible')
  cy.findByText('Demo account', { exact: true }).click()

  cy.findByRole('img', { name: 'Deriv X logo' })
    .scrollIntoView()
    .should('be.visible')
  cy.findByRole('heading', { name: 'Get trading with Deriv X' })
    .scrollIntoView()
    .should('be.visible')
...
describe('QATEST-1571 - validate the derivx page', () => {
  it(
    'should be able to navigate to derivx page from home page and validate the page content and links for ROW in desktop',
  ...
  it(
    'should be able to navigate to derivx page from home page and validate the page content and links for ROW in mobile',
    { tags: ['@full-tests', '@row-tests'] },
    () => {
