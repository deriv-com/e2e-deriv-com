...
const derivedindicesSyntheticsCFDArray = [
  'DSI10',
  'DSI20',
  'DSI30',
  'DEX 600UP',
...
const derivedindicesSyntheticsOptionsArray = [
  'Volatility 10 (1s) Index',
  'Volatility 25 (1s) Index',
  'Volatility 50 (1s) Index',
  'Volatility 75 (1s) Index',
...
const derivedindicesBasketArray = [
  'Gold Basket',
  'AUD Basket',
  'EUR Basket',
  'GBP Basket',
...
const forexBasketArray = [
  'AUD Basket',
  'EUR Basket',
  'GBP Basket',
  'USD Basket',
]
const derivedFXArray = [
  'GBP/USD DFX10 Index',
  'EUR/USD DFX10 Index',
  'AUD/USD DFX10 Index',
  'USD/JPY DFX10 Index',
...
function validateInstruments(array) {
  cy.findAllByRole('img', { name: 'symbol' })
    .siblings('p')
    .each(($p) => {
...
Cypress.Commands.add('c_validateMarkets', (site) => {
  if (site == 'row') {
    ...
    cy.findByRole('tab', { name: 'ETFs' }).click()
    cy.findByText('MSCI Emerging Markets (EEM)')
      .scrollIntoView()
      .should('be.visible')
    cy.findByRole('tab', { name: 'Stocks & indices' }).click()
    cy.findByText('Apple (APPL)').scrollIntoView().should('be.visible')
    ...
    cy.findByRole('tab', { name: 'Commodities' }).click()
    cy.findByText('West Texas Intermediate')
      .scrollIntoView()
      .should('be.visible')
  }
})
...
function etf_row() {
  ...
  cy.findByRole('heading', { name: 'CFDs' }).should('be.visible')
  cy.get(
    '[class^="at-flex instrumental-table-module--instrumental_left_item--"]'
  )
    .find('p')
    .should('have.text', 'ETFs')
